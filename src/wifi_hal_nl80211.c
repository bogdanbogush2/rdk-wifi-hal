...
#ifdef EAPOL_OVER_NL
                    if (interface->bss_frames_registered == 1 && interface->bss_allow_recv) {
...
int cookie_handler(struct nl_msg *msg, void *arg)
{
             ...
             void *valid_finish_data)
{
    ...
    //XXX HARDCODE TEST
    int cmd = get_cmd_from_nl_msg(msg);
    if (cmd == NL80211_CMD_SET_WIPHY ||
    ...
    if (g_wifi_hal.config_done == false &&
        (cmd == NL80211_CMD_NEW_KEY || cmd == NL80211_CMD_SET_KEY)) {
        err = 0;
        goto out;
    }
...
struct nl_msg *nl80211_drv_cmd_msg(int nl80211_id, wifi_interface_info_t *intf, int flags, uint8_t cmd)
{
    ...
    if (intf != NULL) {
        ...
        nla_put_u32(msg, NL80211_ATTR_WIPHY, intf->phy_index);
#if HOSTAPD_VERSION >= 211 // 2.11
        wifi_hal_error_print("%s:%d cmd: %d mld: %d\n", __func__, __LINE__, cmd, wifi_hal_is_mld_enabled(intf));
        if (wifi_hal_is_mld_enabled(intf)) {
            wifi_hal_error_print("%s:%d link id: %d\n", __func__, __LINE__, wifi_hal_get_mld_link_id(intf));
            if (nla_put_u8(msg, NL80211_ATTR_MLO_LINK_ID, wifi_hal_get_mld_link_id(intf)) < 0) {
                wifi_hal_error_print("%s:%d Failed to put MLO link id attribute\n",
                    __func__, __LINE__);
                nlmsg_free(msg);
                return NULL;
            }
...
{
    wifi_radio_info_t *radio = get_radio_by_rdk_index(0);
    if (radio == NULL) {
        wifi_hal_error_print("%s:%d: Error getting radio by RDK index\n", __func__, __LINE__);
        return -1;
    }
    wifi_interface_info_t *interface = get_private_vap_interface(radio);
    if (interface == NULL) {
        wifi_hal_error_print("%s:%d: Error getting private vap interface\n", __func__, __LINE__);
        return -1;
    }
...
int nl80211_init_primary_interfaces()
{
    ...
    // XXX HARDCODE TEST
    char set_interface[] = { 0x24, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00, 0x33, 0xd3, 0x6a, 0x62,
        0x60, 0x0c, 0x00, 0x00, 0x06, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00, 0x11, 0x00, 0x00,
        0x00, 0x08, 0x00, 0x05, 0x00, 0x03, 0x00, 0x00, 0x00 };
    if (send_netlink_raw((unsigned char *)set_interface, sizeof(set_interface)) < 0) {
        wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
    } else {
        wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
    }
    sleep(0.1);

    char add_link0[] = { 0x30, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00, 0x39, 0xd3, 0x6a, 0x62,
        0x60, 0x0c, 0x00, 0x00, 0x94, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00, 0x11, 0x00, 0x00,
        0x00, 0x05, 0x00, 0x39, 0x01, 0x00, 0x00, 0x00, 0x00, 0x0a, 0x00, 0x06, 0x00, 0x00, 0x11,
        0x22, 0x33, 0x44, 0x00, 0x00, 0x00 };
    if (send_netlink_raw((unsigned char *)add_link0, sizeof(add_link0)) < 0) {
        wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
    } else {
        wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
    }
    ...
    char set_reg0[] = { 0x1c, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00, 0x3b, 0xd3, 0x6a, 0x62, 0x60,
        0x0c, 0x00, 0x00, 0x1b, 0x00, 0x00, 0x00, 0x07, 0x00, 0x21, 0x00, 0x55, 0x53, 0x00, 0x00 };
    if (send_netlink_raw((unsigned char *)set_reg0, sizeof(set_reg0)) < 0) {
        wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
    } else {
        wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
    }
    ...
    char set_wiphy0[] = { 0x34, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00, 0x41, 0xd3, 0x6a, 0x62,
        0x60, 0x0c, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00, 0x11, 0x00, 0x00,
        0x00, 0x08, 0x00, 0x26, 0x00, 0x6c, 0x09, 0x00, 0x00, 0x08, 0x00, 0x27, 0x00, 0x03, 0x00,
        0x00, 0x00, 0x05, 0x00, 0x39, 0x01, 0x00, 0x00, 0x00, 0x00 };
    if (send_netlink_raw((unsigned char *)set_wiphy0, sizeof(set_wiphy0)) < 0) {
        wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
    } else {
        wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
    }
    ...
    char del_station0[] = { 0x24, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00, 0x42, 0xd3, 0x6a, 0x62,
        0x60, 0x0c, 0x00, 0x00, 0x14, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00, 0x11, 0x00, 0x00,
        0x00, 0x05, 0x00, 0x39, 0x01, 0x00, 0x00, 0x00, 0x00 };
    if (send_netlink_raw((unsigned char *)del_station0, sizeof(del_station0)) < 0) {
        wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
    } else {
        wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
    }
    ...
    char set_qos_map0[] = { 0x34, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00, 0x43, 0xd3, 0x6a, 0x62,
        0x60, 0x0c, 0x00, 0x00, 0x68, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00, 0x11, 0x00, 0x00,
        0x00, 0x16, 0x00, 0xc7, 0x00, 0x00, 0x00, 0x02, 0x10, 0x01, 0x01, 0xff, 0xff, 0x12, 0x16,
        0x18, 0x26, 0x28, 0x28, 0x2c, 0x2e, 0x30, 0x38, 0x00, 0x00 };
    if (send_netlink_raw((unsigned char *)set_qos_map0, sizeof(set_qos_map0)) < 0) {
        wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
    } else {
        wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
    }
    ...
    char set_channel0[] = { 0x34, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00, 0x45, 0xd3, 0x6a, 0x62,
        0x60, 0x0c, 0x00, 0x00, 0x41, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00, 0x11, 0x00, 0x00,
        0x00, 0x08, 0x00, 0x26, 0x00, 0x6c, 0x09, 0x00, 0x00, 0x08, 0x00, 0x27, 0x00, 0x03, 0x00,
        0x00, 0x00, 0x05, 0x00, 0x39, 0x01, 0x00, 0x00, 0x00, 0x00 };
    if (send_netlink_raw((unsigned char *)set_channel0, sizeof(set_channel0)) < 0) {
        wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
    } else {
        wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
    }
    ...
    char start_ap0[] = {  };

    if (send_netlink_raw((unsigned char *)start_ap0, sizeof(start_ap0)) < 0) {
        wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
    } else {
        wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
    }
    ...
    char start_ap02[] = { 0x48, 0x00, 0x00, 0x00, 0x19, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x0f, 0x01, 0x00, 0x00, 0x08, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x08, 0x00, 0x03, 0x00, 0x11, 0x00, 0x00, 0x00, 0x0c, 0x00, 0x99, 0x00, 0x04, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x0f, 0x00, 0x34, 0x00, 0x4d, 0x54, 0x37, 0x36, 0x5f,
        0x41, 0x50, 0x5f, 0x4d, 0x4c, 0x44, 0x00, 0x05, 0x00, 0x39, 0x01, 0x00, 0x00, 0x00, 0x00 };

    if (send_netlink_raw((unsigned char *)start_ap02, sizeof(start_ap02)) < 0) {
        wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
    } else {
        wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
    }
    ...
    char set_bss0[] = { 0x54, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00, 0x46, 0xd3, 0x6a, 0x62,
        0x60, 0x0c, 0x00, 0x00, 0x19, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00, 0x11, 0x00, 0x00,
        0x00, 0x05, 0x00, 0x1c, 0x00, 0x00, 0x00, 0x00, 0x00, 0x05, 0x00, 0x1d, 0x00, 0x01, 0x00,
        0x00, 0x00, 0x05, 0x00, 0x1e, 0x00, 0x01, 0x00, 0x00, 0x00, 0x06, 0x00, 0x6d, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x05, 0x00, 0x60, 0x00, 0x01, 0x00, 0x00, 0x00, 0x07, 0x00, 0x24, 0x00,
        0x0c, 0x18, 0x30, 0x00, 0x05, 0x00, 0x39, 0x01, 0x00, 0x00, 0x00, 0x00 };

    if (send_netlink_raw((unsigned char *)set_bss0, sizeof(set_bss0)) < 0) {
        wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
    } else {
        wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
    }
    ...
    char set_mcast0[] = { 0x1c, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00, 0x47, 0xd3, 0x6a, 0x62,
        0x60, 0x0c, 0x00, 0x00, 0x79, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00, 0x11, 0x00, 0x00,
        0x00 };
    if (send_netlink_raw((unsigned char *)set_mcast0, sizeof(set_mcast0)) < 0) {
        wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
    } else {
        wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
    }
...
char deauth[] = {
  0x4c, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00,
  0x48, 0xd3, 0x6a, 0x62, 0x60, 0x0c, 0x00, 0x00,
  0x3b, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00,
  0x11, 0x00, 0x00, 0x00, 0x05, 0x00, 0x39, 0x01,
  0x00, 0x00, 0x00, 0x00, 0x08, 0x00, 0x26, 0x00,
  0x6c, 0x09, 0x00, 0x00, 0x1e, 0x00, 0x33, 0x00,
  0xc0, 0x00, 0x00, 0x00, 0xff, 0xff, 0xff, 0xff,
  0xff, 0xff, 0x00, 0x11, 0x22, 0x33, 0x44, 0x55,
  0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x00, 0x00,
  0x02, 0x00, 0x00, 0x00
};
    if (send_netlink_raw((unsigned char *)deauth, sizeof(deauth)) < 0) {
        wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
    } else {
        wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
    }

    char new_key0[] = { 0x4c, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00, 0x49, 0xd3, 0x6a, 0x62,
        0x60, 0x0c, 0x00, 0x00, 0x0b, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00, 0x11, 0x00, 0x00,
        0x00, 0x28, 0x00, 0x50, 0x00, 0x14, 0x00, 0x01, 0x00, 0xc6, 0xba, 0xae, 0x70, 0xc9, 0xe6,
        0x7c, 0x84, 0x19, 0xcf, 0xcc, 0x7c, 0xbf, 0x1a, 0xb3, 0x9b, 0x08, 0x00, 0x03, 0x00, 0x04,
        0xac, 0x0f, 0x00, 0x05, 0x00, 0x02, 0x00, 0x01, 0x00, 0x00, 0x00, 0x05, 0x00, 0x39, 0x01,
        0x00, 0x00, 0x00, 0x00 };
    if (send_netlink_raw((unsigned char *)new_key0, sizeof(new_key0)) < 0) {
        wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
    } else {
        wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
    }
    ...
    char set_key0[] = { 0x3c, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00, 0x4a, 0xd3, 0x6a, 0x62,
        0x60, 0x0c, 0x00, 0x00, 0x0a, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00, 0x11, 0x00, 0x00,
        0x00, 0x18, 0x00, 0x50, 0x00, 0x05, 0x00, 0x02, 0x00, 0x01, 0x00, 0x00, 0x00, 0x04, 0x00,
        0x05, 0x00, 0x08, 0x00, 0x08, 0x80, 0x04, 0x00, 0x02, 0x00, 0x05, 0x00, 0x39, 0x01, 0x00,
        0x00, 0x00, 0x00 };
    if (send_netlink_raw((unsigned char *)set_key0, sizeof(set_key0)) < 0) {
        wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
    } else {
        wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
    }
    ...
    char new_key02[] = { 0x4c, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00, 0x4b, 0xd3, 0x6a, 0x62,
        0x60, 0x0c, 0x00, 0x00, 0x0b, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00, 0x11, 0x00, 0x00,
        0x00, 0x28, 0x00, 0x50, 0x00, 0x14, 0x00, 0x01, 0x00, 0x58, 0x4a, 0xb5, 0x80, 0xac, 0x97,
        0xea, 0x9a, 0x96, 0xa3, 0xf9, 0xcd, 0xc8, 0x7a, 0xb3, 0xb9, 0x08, 0x00, 0x03, 0x00, 0x06,
        0xac, 0x0f, 0x00, 0x05, 0x00, 0x02, 0x00, 0x04, 0x00, 0x00, 0x00, 0x05, 0x00, 0x39, 0x01,
        0x00, 0x00, 0x00, 0x00 };
    if (send_netlink_raw((unsigned char *)new_key02, sizeof(new_key02)) < 0) {
        wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
    } else {
        wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
    }
    ...
    char set_key02[] = { 0x3c, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00, 0x4c, 0xd3, 0x6a, 0x62,
        0x60, 0x0c, 0x00, 0x00, 0x0a, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00, 0x11, 0x00, 0x00,
        0x00, 0x18, 0x00, 0x50, 0x00, 0x05, 0x00, 0x02, 0x00, 0x04, 0x00, 0x00, 0x00, 0x04, 0x00,
        0x06, 0x00, 0x08, 0x00, 0x08, 0x80, 0x04, 0x00, 0x02, 0x00, 0x05, 0x00, 0x39, 0x01, 0x00,
        0x00, 0x00, 0x00 };
    if (send_netlink_raw((unsigned char *)set_key02, sizeof(set_key02)) < 0) {
        wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
    } else {
        wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
    }
    ...
    char set_wiphi02[] = { 0x54, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00, 0x4d, 0xd3, 0x6a, 0x62,
        0x60, 0x0c, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00, 0x11, 0x00, 0x00,
        0x00, 0x30, 0x00, 0x25, 0x80, 0x2c, 0x00, 0x01, 0x80, 0x05, 0x00, 0x01, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x06, 0x00, 0x02, 0x00, 0x2f, 0x00, 0x00, 0x00, 0x06, 0x00, 0x03, 0x00, 0x03,
        0x00, 0x00, 0x00, 0x06, 0x00, 0x04, 0x00, 0x07, 0x00, 0x00, 0x00, 0x05, 0x00, 0x05, 0x00,
        0x01, 0x00, 0x00, 0x00, 0x05, 0x00, 0x39, 0x01, 0x00, 0x00, 0x00, 0x00 };
    if (send_netlink_raw((unsigned char *)set_wiphi02, sizeof(set_wiphi02)) < 0) {
        wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
    } else {
        wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
    }
    ...
    char set_wiphi03[] = { 0x54, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00, 0x4e, 0xd3, 0x6a, 0x62,
        0x60, 0x0c, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00, 0x11, 0x00, 0x00,
        0x00, 0x30, 0x00, 0x25, 0x80, 0x2c, 0x00, 0x01, 0x80, 0x05, 0x00, 0x01, 0x00, 0x01, 0x00,
        0x00, 0x00, 0x06, 0x00, 0x02, 0x00, 0x5e, 0x00, 0x00, 0x00, 0x06, 0x00, 0x03, 0x00, 0x07,
        0x00, 0x00, 0x00, 0x06, 0x00, 0x04, 0x00, 0x0f, 0x00, 0x00, 0x00, 0x05, 0x00, 0x05, 0x00,
        0x01, 0x00, 0x00, 0x00, 0x05, 0x00, 0x39, 0x01, 0x00, 0x00, 0x00, 0x00 };
    if (send_netlink_raw((unsigned char *)set_wiphi03, sizeof(set_wiphi03)) < 0) {
        wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
    } else {
        wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
    }
    ...
    char set_wiphi04[] = { 0x54, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00, 0x4f, 0xd3, 0x6a, 0x62,
        0x60, 0x0c, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00, 0x11, 0x00, 0x00,
        0x00, 0x30, 0x00, 0x25, 0x80, 0x2c, 0x00, 0x01, 0x80, 0x05, 0x00, 0x01, 0x00, 0x02, 0x00,
        0x00, 0x00, 0x06, 0x00, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x06, 0x00, 0x03, 0x00, 0x0f,
        0x00, 0x00, 0x00, 0x06, 0x00, 0x04, 0x00, 0x3f, 0x00, 0x00, 0x00, 0x05, 0x00, 0x05, 0x00,
        0x03, 0x00, 0x00, 0x00, 0x05, 0x00, 0x39, 0x01, 0x00, 0x00, 0x00, 0x00 };

    if (send_netlink_raw((unsigned char *)set_wiphi04, sizeof(set_wiphi04)) < 0) {
        wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
    } else {
        wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
    }
    ...
    char set_wiphi05[] = { 0x54, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00, 0x50, 0xd3, 0x6a, 0x62,
        0x60, 0x0c, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00, 0x11, 0x00, 0x00,
        0x00, 0x30, 0x00, 0x25, 0x80, 0x2c, 0x00, 0x01, 0x80, 0x05, 0x00, 0x01, 0x00, 0x03, 0x00,
        0x00, 0x00, 0x06, 0x00, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x06, 0x00, 0x03, 0x00, 0x0f,
        0x00, 0x00, 0x00, 0x06, 0x00, 0x04, 0x00, 0xff, 0x03, 0x00, 0x00, 0x05, 0x00, 0x05, 0x00,
        0x07, 0x00, 0x00, 0x00, 0x05, 0x00, 0x39, 0x01, 0x00, 0x00, 0x00, 0x00 };

    if (send_netlink_raw((unsigned char *)set_wiphi05, sizeof(set_wiphi05)) < 0) {
        wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
    } else {
        wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
    }
    ...
    char vendor0_196[] = { 0x48, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00, 0x51, 0xd3, 0x6a, 0x62,
        0x60, 0x0c, 0x00, 0x00, 0x67, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00, 0x11, 0x00, 0x00,
        0x00, 0x08, 0x00, 0xc3, 0x00, 0xe7, 0x0c, 0x00, 0x00, 0x08, 0x00, 0xc4, 0x00, 0xc7, 0x00,
        0x00, 0x00, 0x1c, 0x00, 0xc5, 0x80, 0x05, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x05,
        0x00, 0x02, 0x00, 0x01, 0x00, 0x00, 0x00, 0x05, 0x00, 0x06, 0x00, 0xfa, 0x00, 0x00, 0x00 };
    if (send_netlink_raw((unsigned char *)vendor0_196, sizeof(vendor0_196)) < 0) {
        wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
    } else {
        wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
    }
...
char vendor0_196_2[] = {
  0x40, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00,
  0x52, 0xd3, 0x6a, 0x62, 0x60, 0x0c, 0x00, 0x00,
  0x67, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00,
  0x11, 0x00, 0x00, 0x00, 0x08, 0x00, 0xc3, 0x00,
  0xe7, 0x0c, 0x00, 0x00, 0x08, 0x00, 0xc4, 0x00,
  0xc5, 0x00, 0x00, 0x00, 0x14, 0x00, 0xc5, 0x80,
  0x05, 0x00, 0x01, 0x00, 0x0f, 0x00, 0x00, 0x00,
  0x05, 0x00, 0x04, 0x00, 0x00, 0x00, 0x00, 0x00
};

    if (send_netlink_raw((unsigned char *)vendor0_196_2, sizeof(vendor0_196_2)) < 0) {
        wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
    } else {
        wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
    }
...
char vendor0_196_3[] = {
  0x38, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00,
  0x53, 0xd3, 0x6a, 0x62, 0x60, 0x0c, 0x00, 0x00,
  0x67, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00,
  0x11, 0x00, 0x00, 0x00, 0x08, 0x00, 0xc3, 0x00,
  0xe7, 0x0c, 0x00, 0x00, 0x08, 0x00, 0xc4, 0x00,
  0xc8, 0x00, 0x00, 0x00, 0x0c, 0x00, 0xc5, 0x80,
  0x05, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00
};
    if (send_netlink_raw((unsigned char *)vendor0_196_3, sizeof(vendor0_196_3)) < 0) {
        wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
    } else {
        wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
    }
...
char vendor0_196_4[] = {
  0x38, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00,
  0x54, 0xd3, 0x6a, 0x62, 0x60, 0x0c, 0x00, 0x00,
  0x67, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00,
  0x11, 0x00, 0x00, 0x00, 0x08, 0x00, 0xc3, 0x00,
  0xe7, 0x0c, 0x00, 0x00, 0x08, 0x00, 0xc4, 0x00,
  0xc9, 0x00, 0x00, 0x00, 0x0c, 0x00, 0xc5, 0x80,
  0x05, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00
};

    if (send_netlink_raw((unsigned char *)vendor0_196_4, sizeof(vendor0_196_4)) < 0) {
        wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
    } else {
        wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
    }
...
char vendor0_196_5[] = {
  0x38, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00,
  0x55, 0xd3, 0x6a, 0x62, 0x60, 0x0c, 0x00, 0x00,
  0x67, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00,
  0x11, 0x00, 0x00, 0x00, 0x08, 0x00, 0xc3, 0x00,
  0xe7, 0x0c, 0x00, 0x00, 0x08, 0x00, 0xc4, 0x00,
  0xc4, 0x00, 0x00, 0x00, 0x0c, 0x00, 0xc5, 0x80,
  0x05, 0x00, 0x07, 0x00, 0x01, 0x00, 0x00, 0x00
};

    if (send_netlink_raw((unsigned char *)vendor0_196_5, sizeof(vendor0_196_5)) < 0) {
        wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
    } else {
        wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
    }
...
char vendor0_196_6[] = {
  0x58, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00,
  0x56, 0xd3, 0x6a, 0x62, 0x60, 0x0c, 0x00, 0x00,
  0x67, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00,
  0x11, 0x00, 0x00, 0x00, 0x08, 0x00, 0xc3, 0x00,
  0xe7, 0x0c, 0x00, 0x00, 0x08, 0x00, 0xc4, 0x00,
  0xce, 0x00, 0x00, 0x00, 0x2c, 0x00, 0xc5, 0x80,
  0x05, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00,
  ...
  0x05, 0x00, 0x06, 0x00, 0x01, 0x00, 0x00, 0x00
    ...
    if (send_netlink_raw((unsigned char *)vendor0_196_6, sizeof(vendor0_196_6)) < 0) {
        wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
    } else {
        wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
    }
...
char add_link1[] = {
  0x30, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00,
  0x57, 0xd3, 0x6a, 0x62, 0x60, 0x0c, 0x00, 0x00,
  0x94, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00,
  0x11, 0x00, 0x00, 0x00, 0x05, 0x00, 0x39, 0x01,
  0x01, 0x00, 0x00, 0x00, 0x0a, 0x00, 0x06, 0x00,
  0x00, 0x11, 0x22, 0x33, 0x44, 0x01, 0x00, 0x00
};

if (send_netlink_raw((unsigned char *)add_link1, sizeof(add_link1)) < 0) {
    wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
} else {
    wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
}
...
char set_reg1[] = {
  0x1c, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00,
  0x59, 0xd3, 0x6a, 0x62, 0x60, 0x0c, 0x00, 0x00,
  0x1b, 0x00, 0x00, 0x00, 0x07, 0x00, 0x21, 0x00,
  0x55, 0x53, 0x00, 0x00
};
sleep(0.1);

if (send_netlink_raw((unsigned char *)set_reg1, sizeof(set_reg1)) < 0) {
    wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
} else {
    wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
}
...
char set_wiphy1[] = {
  0x3c, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00,
  0x5f, 0xd3, 0x6a, 0x62, 0x60, 0x0c, 0x00, 0x00,
  0x02, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00,
  0x11, 0x00, 0x00, 0x00, 0x08, 0x00, 0x26, 0x00,
  0x3c, 0x14, 0x00, 0x00, 0x08, 0x00, 0x9f, 0x00,
  0x03, 0x00, 0x00, 0x00, 0x08, 0x00, 0xa0, 0x00,
  0x5a, 0x14, 0x00, 0x00, 0x05, 0x00, 0x39, 0x01,
  0x01, 0x00, 0x00, 0x00
};
sleep(0.1);

if (send_netlink_raw((unsigned char *)set_wiphy1, sizeof(set_wiphy1)) < 0) {
    wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
} else {
    wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
}
...
char set_qos1[] = {
  0x34, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00,
  0x60, 0xd3, 0x6a, 0x62, 0x60, 0x0c, 0x00, 0x00,
  0x68, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00,
  0x11, 0x00, 0x00, 0x00, 0x16, 0x00, 0xc7, 0x00,
  0x00, 0x00, 0x02, 0x10, 0x01, 0x01, 0xff, 0xff,
  0x12, 0x16, 0x18, 0x26, 0x28, 0x28, 0x2c, 0x2e,
  0x30, 0x38, 0x00, 0x00
};

if (send_netlink_raw((unsigned char *)set_qos1, sizeof(set_qos1)) < 0) {
    wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
} else {
    wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
}
...
char set_channel1[] = {
  0x3c, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00,
  0x61, 0xd3, 0x6a, 0x62, 0x60, 0x0c, 0x00, 0x00,
  0x41, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00,
  0x11, 0x00, 0x00, 0x00, 0x08, 0x00, 0x26, 0x00,
  0x3c, 0x14, 0x00, 0x00, 0x08, 0x00, 0x9f, 0x00,
  0x03, 0x00, 0x00, 0x00, 0x08, 0x00, 0xa0, 0x00,
  0x5a, 0x14, 0x00, 0x00, 0x05, 0x00, 0x39, 0x01,
...
if (send_netlink_raw((unsigned char *)set_channel1, sizeof(set_channel1)) < 0) {
    wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
} else {
    wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
}
...
char start_ap1[] = {
  
};


if (send_netlink_raw((unsigned char *)start_ap1, sizeof(start_ap1)) < 0) {
    wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
} else {
    wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
}
...
char start_ap12[] = {
  0x48, 0x00, 0x00, 0x00, 0x19, 0x00, 0x00, 0x00,
  0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
  0x0f, 0x01, 0x00, 0x00, 0x08, 0x00, 0x01, 0x00,
  0x00, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00,
  0x11, 0x00, 0x00, 0x00, 0x0c, 0x00, 0x99, 0x00,
  0x04, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
  0x0f, 0x00, 0x34, 0x00, 0x4d, 0x54, 0x37, 0x36,
  0x5f, 0x41, 0x50, 0x5f, 0x4d, 0x4c, 0x44, 0x00,
  0x05, 0x00, 0x39, 0x01, 0x01, 0x00, 0x00, 0x00
};

if (send_netlink_raw((unsigned char *)start_ap12, sizeof(start_ap12)) < 0) {
    wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
} else {
    wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
}
...
char set_bss1[] = {
  0x4c, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00,
  0x62, 0xd3, 0x6a, 0x62, 0x60, 0x0c, 0x00, 0x00,
  0x19, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00,
  0x11, 0x00, 0x00, 0x00, 0x05, 0x00, 0x1c, 0x00,
  0x00, 0x00, 0x00, 0x00, 0x05, 0x00, 0x1d, 0x00,
  0x01, 0x00, 0x00, 0x00, 0x06, 0x00, 0x6d, 0x00,
  0x00, 0x00, 0x00, 0x00, 0x05, 0x00, 0x60, 0x00,
  0x01, 0x00, 0x00, 0x00, 0x07, 0x00, 0x24, 0x00,
  0x0c, 0x18, 0x30, 0x00, 0x05, 0x00, 0x39, 0x01,
  0x01, 0x00, 0x00, 0x00
};

if (send_netlink_raw((unsigned char *)set_bss1, sizeof(set_bss1)) < 0) {
    wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
} else {
    wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
}
...
char set_mcast1[] = {
  0x1c, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00,
  0x63, 0xd3, 0x6a, 0x62, 0x60, 0x0c, 0x00, 0x00,
  0x79, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00,
  0x11, 0x00, 0x00, 0x00
};

if (send_netlink_raw((unsigned char *)set_mcast1, sizeof(set_mcast1)) < 0) {
    wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
} else {
    wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
}
...
char set_new_key1[] = {
  0x4c, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00,
  0x64, 0xd3, 0x6a, 0x62, 0x60, 0x0c, 0x00, 0x00,
  0x0b, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00,
  0x11, 0x00, 0x00, 0x00, 0x28, 0x00, 0x50, 0x00,
  0x14, 0x00, 0x01, 0x00, 0x3d, 0x94, 0x11, 0x38,
  0xe2, 0x3b, 0xa3, 0x81, 0x25, 0x05, 0x2d, 0x44,
  0xd6, 0x64, 0xa6, 0x4a, 0x08, 0x00, 0x03, 0x00,
  0x04, 0xac, 0x0f, 0x00, 0x05, 0x00, 0x02, 0x00,
  0x01, 0x00, 0x00, 0x00, 0x05, 0x00, 0x39, 0x01,
  0x01, 0x00, 0x00, 0x00
};

if (send_netlink_raw((unsigned char *)set_new_key1, sizeof(set_new_key1)) < 0) {
    wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
} else {
    wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
}
...
char set_key1[] = {
  0x3c, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00,
  0x65, 0xd3, 0x6a, 0x62, 0x60, 0x0c, 0x00, 0x00,
  0x0a, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00,
  0x11, 0x00, 0x00, 0x00, 0x18, 0x00, 0x50, 0x00,
  0x05, 0x00, 0x02, 0x00, 0x01, 0x00, 0x00, 0x00,
  0x04, 0x00, 0x05, 0x00, 0x08, 0x00, 0x08, 0x80,
  0x04, 0x00, 0x02, 0x00, 0x05, 0x00, 0x39, 0x01,
  0x01, 0x00, 0x00, 0x00
};

if (send_netlink_raw((unsigned char *)set_key1, sizeof(set_key1)) < 0) {
    wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
} else {
    wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
}
...
char new_key12[] = {
  0x4c, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00,
  0x66, 0xd3, 0x6a, 0x62, 0x60, 0x0c, 0x00, 0x00,
  0x0b, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00,
  0x11, 0x00, 0x00, 0x00, 0x28, 0x00, 0x50, 0x00,
  0x14, 0x00, 0x01, 0x00, 0x9a, 0x48, 0xdd, 0x69,
  0xd3, 0x3e, 0xf7, 0x66, 0x8a, 0x9a, 0x92, 0x49,
  0xbc, 0xd8, 0x9c, 0x0d, 0x08, 0x00, 0x03, 0x00,
  0x06, 0xac, 0x0f, 0x00, 0x05, 0x00, 0x02, 0x00,
  0x04, 0x00, 0x00, 0x00, 0x05, 0x00, 0x39, 0x01,
  0x01, 0x00, 0x00, 0x00
};

if (send_netlink_raw((unsigned char *)new_key12, sizeof(new_key12)) < 0) {
    wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
} else {
    wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
}
...
char set_key12[] = {
  0x3c, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00,
  0x67, 0xd3, 0x6a, 0x62, 0x60, 0x0c, 0x00, 0x00,
  0x0a, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00,
  0x11, 0x00, 0x00, 0x00, 0x18, 0x00, 0x50, 0x00,
  0x05, 0x00, 0x02, 0x00, 0x04, 0x00, 0x00, 0x00,
  0x04, 0x00, 0x06, 0x00, 0x08, 0x00, 0x08, 0x80,
  0x04, 0x00, 0x02, 0x00, 0x05, 0x00, 0x39, 0x01,
  0x01, 0x00, 0x00, 0x00
};
sleep(0.1);

if (send_netlink_raw((unsigned char *)set_key12, sizeof(set_key12)) < 0) {
    wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
} else {
    wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
}
...
char set_wiphy12[] = {
  0x54, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00,
  0x68, 0xd3, 0x6a, 0x62, 0x60, 0x0c, 0x00, 0x00,
  0x02, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00,
  0x11, 0x00, 0x00, 0x00, 0x30, 0x00, 0x25, 0x80,
  0x2c, 0x00, 0x01, 0x80, 0x05, 0x00, 0x01, 0x00,
  0x00, 0x00, 0x00, 0x00, 0x06, 0x00, 0x02, 0x00,
  0x2f, 0x00, 0x00, 0x00, 0x06, 0x00, 0x03, 0x00,
  0x03, 0x00, 0x00, 0x00, 0x06, 0x00, 0x04, 0x00,
  0x07, 0x00, 0x00, 0x00, 0x05, 0x00, 0x05, 0x00,
  0x01, 0x00, 0x00, 0x00, 0x05, 0x00, 0x39, 0x01,
  0x01, 0x00, 0x00, 0x00
};


if (send_netlink_raw((unsigned char *)set_wiphy12, sizeof(set_wiphy12)) < 0) {
    wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
} else {
    wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
}
...
char set_wiphy13[] = {
  0x54, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00,
  0x69, 0xd3, 0x6a, 0x62, 0x60, 0x0c, 0x00, 0x00,
  0x02, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00,
  0x11, 0x00, 0x00, 0x00, 0x30, 0x00, 0x25, 0x80,
  0x2c, 0x00, 0x01, 0x80, 0x05, 0x00, 0x01, 0x00,
  0x01, 0x00, 0x00, 0x00, 0x06, 0x00, 0x02, 0x00,
  0x5e, 0x00, 0x00, 0x00, 0x06, 0x00, 0x03, 0x00,
  0x07, 0x00, 0x00, 0x00, 0x06, 0x00, 0x04, 0x00,
  0x0f, 0x00, 0x00, 0x00, 0x05, 0x00, 0x05, 0x00,
  0x01, 0x00, 0x00, 0x00, 0x05, 0x00, 0x39, 0x01,
  0x01, 0x00, 0x00, 0x00
};

if (send_netlink_raw((unsigned char *)set_wiphy13, sizeof(set_wiphy13)) < 0) {
    wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
} else {
    wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
}
...
char set_wiphy14[] = {
  0x54, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00,
  0x6a, 0xd3, 0x6a, 0x62, 0x60, 0x0c, 0x00, 0x00,
  0x02, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00,
  0x11, 0x00, 0x00, 0x00, 0x30, 0x00, 0x25, 0x80,
  0x2c, 0x00, 0x01, 0x80, 0x05, 0x00, 0x01, 0x00,
  0x02, 0x00, 0x00, 0x00, 0x06, 0x00, 0x02, 0x00,
  0x00, 0x00, 0x00, 0x00, 0x06, 0x00, 0x03, 0x00,
  0x0f, 0x00, 0x00, 0x00, 0x06, 0x00, 0x04, 0x00,
  0x3f, 0x00, 0x00, 0x00, 0x05, 0x00, 0x05, 0x00,
  0x03, 0x00, 0x00, 0x00, 0x05, 0x00, 0x39, 0x01,
  0x01, 0x00, 0x00, 0x00
};

if (send_netlink_raw((unsigned char *)set_wiphy14, sizeof(set_wiphy14)) < 0) {
    wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
} else {
    wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
}
...
char set_wiphy15[] = {
  0x54, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00,
  0x6b, 0xd3, 0x6a, 0x62, 0x60, 0x0c, 0x00, 0x00,
  0x02, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00,
  0x11, 0x00, 0x00, 0x00, 0x30, 0x00, 0x25, 0x80,
  0x2c, 0x00, 0x01, 0x80, 0x05, 0x00, 0x01, 0x00,
  0x03, 0x00, 0x00, 0x00, 0x06, 0x00, 0x02, 0x00,
  0x00, 0x00, 0x00, 0x00, 0x06, 0x00, 0x03, 0x00,
  0x0f, 0x00, 0x00, 0x00, 0x06, 0x00, 0x04, 0x00,
  0xff, 0x03, 0x00, 0x00, 0x05, 0x00, 0x05, 0x00,
  0x07, 0x00, 0x00, 0x00, 0x05, 0x00, 0x39, 0x01,
  0x01, 0x00, 0x00, 0x00
};

if (send_netlink_raw((unsigned char *)set_wiphy15, sizeof(set_wiphy15)) < 0) {
    wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
} else {
    wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
}
...
char set_vendor_196_1_0[] = {
  0x48, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00,
  0x6c, 0xd3, 0x6a, 0x62, 0x60, 0x0c, 0x00, 0x00,
  0x67, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00,
  0x11, 0x00, 0x00, 0x00, 0x08, 0x00, 0xc3, 0x00,
  0xe7, 0x0c, 0x00, 0x00, 0x08, 0x00, 0xc4, 0x00,
  0xcc, 0x00, 0x00, 0x00, 0x1c, 0x00, 0xc5, 0x80,
  0x05, 0x00, 0x02, 0x00, 0x01, 0x00, 0x00, 0x00,
...
if (send_netlink_raw((unsigned char *)set_vendor_196_1_0, sizeof(set_vendor_196_1_0)) < 0) {
    wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
} else {
    wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
}
...
char set_vendor_196_1_1[] = {
  0x48, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00,
  0x6d, 0xd3, 0x6a, 0x62, 0x60, 0x0c, 0x00, 0x00,
  0x67, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00,
  0x11, 0x00, 0x00, 0x00, 0x08, 0x00, 0xc3, 0x00,
  0xe7, 0x0c, 0x00, 0x00, 0x08, 0x00, 0xc4, 0x00,
  0xc7, 0x00, 0x00, 0x00, 0x1c, 0x00, 0xc5, 0x80,
  0x05, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00,
  0x05, 0x00, 0x02, 0x00, 0x01, 0x00, 0x00, 0x00,
...
if (send_netlink_raw((unsigned char *)set_vendor_196_1_1, sizeof(set_vendor_196_1_1)) < 0) {
    wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
} else {
    wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
}
...
char set_vendor_196_1_2[] = {
  0x40, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00,
  0x6e, 0xd3, 0x6a, 0x62, 0x60, 0x0c, 0x00, 0x00,
  0x67, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00,
  0x11, 0x00, 0x00, 0x00, 0x08, 0x00, 0xc3, 0x00,
  0xe7, 0x0c, 0x00, 0x00, 0x08, 0x00, 0xc4, 0x00,
  0xc5, 0x00, 0x00, 0x00, 0x14, 0x00, 0xc5, 0x80,
...
if (send_netlink_raw((unsigned char *)set_vendor_196_1_2, sizeof(set_vendor_196_1_2)) < 0) {
    wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
} else {
    wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
}
...
char set_vendor_196_1_3[] = {
  0x38, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00,
  0x6f, 0xd3, 0x6a, 0x62, 0x60, 0x0c, 0x00, 0x00,
  0x67, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00,
  0x11, 0x00, 0x00, 0x00, 0x08, 0x00, 0xc3, 0x00,
  0xe7, 0x0c, 0x00, 0x00, 0x08, 0x00, 0xc4, 0x00,
  0xc8, 0x00, 0x00, 0x00, 0x0c, 0x00, 0xc5, 0x80,
...
if (send_netlink_raw((unsigned char *)set_vendor_196_1_3, sizeof(set_vendor_196_1_3)) < 0) {
    wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
} else {
    wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
}

char set_vendor_196_1_4[] = {
  0x38, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00,
  0x70, 0xd3, 0x6a, 0x62, 0x60, 0x0c, 0x00, 0x00,
  0x67, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00,
  0x11, 0x00, 0x00, 0x00, 0x08, 0x00, 0xc3, 0x00,
  0xe7, 0x0c, 0x00, 0x00, 0x08, 0x00, 0xc4, 0x00,
  0xc9, 0x00, 0x00, 0x00, 0x0c, 0x00, 0xc5, 0x80,
...
if (send_netlink_raw((unsigned char *)set_vendor_196_1_4, sizeof(set_vendor_196_1_4)) < 0) {
    wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
} else {
    wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
}
...
char set_vendor_196_1_5[] = {
  0x38, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00,
  0x71, 0xd3, 0x6a, 0x62, 0x60, 0x0c, 0x00, 0x00,
  0x67, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00,
  0x11, 0x00, 0x00, 0x00, 0x08, 0x00, 0xc3, 0x00,
  0xe7, 0x0c, 0x00, 0x00, 0x08, 0x00, 0xc4, 0x00,
  0xc4, 0x00, 0x00, 0x00, 0x0c, 0x00, 0xc5, 0x80,
...
if (send_netlink_raw((unsigned char *)set_vendor_196_1_5, sizeof(set_vendor_196_1_5)) < 0) {
    wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
} else {
    wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
}

char set_vendor_196_1_6[] = {
  0x58, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00,
  0x72, 0xd3, 0x6a, 0x62, 0x60, 0x0c, 0x00, 0x00,
  0x67, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00,
  0x11, 0x00, 0x00, 0x00, 0x08, 0x00, 0xc3, 0x00,
  0xe7, 0x0c, 0x00, 0x00, 0x08, 0x00, 0xc4, 0x00,
  0xce, 0x00, 0x00, 0x00, 0x2c, 0x00, 0xc5, 0x80,
...
if (send_netlink_raw((unsigned char *)set_vendor_196_1_6, sizeof(set_vendor_196_1_6)) < 0) {
    wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
} else {
    wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
}
...
char add_link2[] = {
  0x30, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00,
  0x74, 0xd3, 0x6a, 0x62, 0x60, 0x0c, 0x00, 0x00,
  0x94, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00,
  0x11, 0x00, 0x00, 0x00, 0x05, 0x00, 0x39, 0x01,
  0x02, 0x00, 0x00, 0x00, 0x0a, 0x00, 0x06, 0x00,
  0x00, 0x11, 0x22, 0x33, 0x44, 0x02, 0x00, 0x00
};


if (send_netlink_raw((unsigned char *)add_link2, sizeof(add_link2)) < 0) {
    wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
} else {
    wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
}
...
char set_reg2[] = {
  0x1c, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00,
  0x76, 0xd3, 0x6a, 0x62, 0x60, 0x0c, 0x00, 0x00,
  0x1b, 0x00, 0x00, 0x00, 0x07, 0x00, 0x21, 0x00,
  0x55, 0x53, 0x00, 0x00
};
...
if (send_netlink_raw((unsigned char *)set_reg2, sizeof(set_reg2)) < 0) {
    wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
} else {
    wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
}
...
char set_wiphy2[] = {
  0x3c, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00,
  0x7c, 0xd3, 0x6a, 0x62, 0x60, 0x0c, 0x00, 0x00,
  0x02, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00,
  0x11, 0x00, 0x00, 0x00, 0x08, 0x00, 0x26, 0x00,
  0xf7, 0x17, 0x00, 0x00, 0x08, 0x00, 0x9f, 0x00,
  0x0d, 0x00, 0x00, 0x00, 0x08, 0x00, 0xa0, 0x00,
  0xd9, 0x17, 0x00, 0x00, 0x05, 0x00, 0x39, 0x01,
  0x02, 0x00, 0x00, 0x00
};

if (send_netlink_raw((unsigned char *)set_wiphy2, sizeof(set_wiphy2)) < 0) {
    wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
} else {
    wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
}
...
char set_qos2[] = {
  0x34, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00,
  0x7d, 0xd3, 0x6a, 0x62, 0x60, 0x0c, 0x00, 0x00,
  0x68, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00,
  0x11, 0x00, 0x00, 0x00, 0x16, 0x00, 0xc7, 0x00,
  0x00, 0x00, 0x02, 0x10, 0x01, 0x01, 0xff, 0xff,
  0x12, 0x16, 0x18, 0x26, 0x28, 0x28, 0x2c, 0x2e,
...
if (send_netlink_raw((unsigned char *)set_qos2, sizeof(set_qos2)) < 0) {
    wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
} else {
    wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
}
...
char set_channel2[] = {
  0x3c, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00,
  0x7e, 0xd3, 0x6a, 0x62, 0x60, 0x0c, 0x00, 0x00,
  0x41, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00,
  0x11, 0x00, 0x00, 0x00, 0x08, 0x00, 0x26, 0x00,
  0xf7, 0x17, 0x00, 0x00, 0x08, 0x00, 0x9f, 0x00,
  0x0d, 0x00, 0x00, 0x00, 0x08, 0x00, 0xa0, 0x00,
...
if (send_netlink_raw((unsigned char *)set_channel2, sizeof(set_channel2)) < 0) {
    wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
} else {
    wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
}
...
char start_ap2[] = {
  
};

if (send_netlink_raw((unsigned char *)start_ap2, sizeof(start_ap2)) < 0) {
    wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
} else {
    wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
}
...
if (send_netlink_raw((unsigned char *)start_ap22, sizeof(start_ap22)) < 0) {
    wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
} else {
    wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
}
...
char set_bss2[] = {
  0x44, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00,
  0x7f, 0xd3, 0x6a, 0x62, 0x60, 0x0c, 0x00, 0x00,
  0x19, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00,
  0x11, 0x00, 0x00, 0x00, 0x05, 0x00, 0x1c, 0x00,
  0x00, 0x00, 0x00, 0x00, 0x05, 0x00, 0x1d, 0x00,
  0x01, 0x00, 0x00, 0x00, 0x05, 0x00, 0x60, 0x00,
  0x01, 0x00, 0x00, 0x00, 0x07, 0x00, 0x24, 0x00,
  0x0c, 0x18, 0x30, 0x00, 0x05, 0x00, 0x39, 0x01,
  0x02, 0x00, 0x00, 0x00
};

if (send_netlink_raw((unsigned char *)set_bss2, sizeof(set_bss2)) < 0) {
    wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
} else {
    wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
}
...
char set_mcast2[] = {
  0x1c, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00,
  0x80, 0xd3, 0x6a, 0x62, 0x60, 0x0c, 0x00, 0x00,
  0x79, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00,
  0x11, 0x00, 0x00, 0x00
};

if (send_netlink_raw((unsigned char *)set_mcast2, sizeof(set_mcast2)) < 0) {
    wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
} else {
    wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
}
...
char set_beacon2[] = {
  0xa0, 0x02, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00,
  0x2a, 0xd3, 0x6a, 0x62, 0x60, 0x0c, 0x80, 0x00,
  0x0e, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00,
  0x11, 0x00, 0x00, 0x00, 0x3f, 0x00, 0x0e, 0x00,
  0x80, 0x00, 0x00, 0x00, 0xff, 0xff, 0xff, 0xff,
  0xff, 0xff, 0x00, 0x11, 0x22, 0x33, 0x44, 0x02,
  0x00, 0x11, 0x22, 0x33, 0x44, 0x02, 0x00, 0x00,
  0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
  0x64, 0x00, 0x31, 0x00, 0x00, 0x0b, 0x4d, 0x54,
  0x37, 0x36, 0x5f, 0x41, 0x50, 0x5f, 0x4d, 0x4c,
  0x44, 0x01, 0x08, 0x8c, 0x12, 0x98, 0x24, 0xb0,
  0x48, 0x60, 0x6c, 0x00, 0x09, 0x01, 0x0f, 0x00,
  0x07, 0x06, 0x55, 0x53, 0x04, 0xc9, 0x89, 0x00,
  ...
  0xff, 0x0c, 0x24, 0xf4, 0x3f, 0x02, 0x01, 0xfc,
  0xff, 0x25, 0x03, 0x27, 0x2f, 0x06, 0xff, 0x02,
  0x27, 0x03, 0xff, 0x0e, 0x26, 0x01, 0x00, 0xa9,
  0x03, 0x20, 0xa9, 0x03, 0x40, 0x75, 0x03, 0x60,
  0x75, 0x03, 0xff, 0x03, 0x3b, 0xba, 0x36, 0xff,
  0x10, 0x6b, 0xb0, 0x01, 0x0d, 0x00, 0x11, 0x22,
  0x33, 0x44, 0x55, 0x02, 0x00, 0x01, 0x00, 0x22,
...
if (send_netlink_raw((unsigned char *)set_beacon2, sizeof(set_beacon2)) < 0) {
    wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
} else {
    wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
}
...
char set_bss22[] = {
  0x44, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00,
  0x81, 0xd3, 0x6a, 0x62, 0x60, 0x0c, 0x00, 0x00,
  0x19, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00,
  0x11, 0x00, 0x00, 0x00, 0x05, 0x00, 0x1c, 0x00,
  0x00, 0x00, 0x00, 0x00, 0x05, 0x00, 0x1d, 0x00,
  0x01, 0x00, 0x00, 0x00, 0x05, 0x00, 0x60, 0x00,
...
if (send_netlink_raw((unsigned char *)set_bss22, sizeof(set_bss22)) < 0) {
    wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
} else {
    wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
}
...
char set_mcast22[] = {
  0x1c, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00,
  0x82, 0xd3, 0x6a, 0x62, 0x60, 0x0c, 0x00, 0x00,
  0x79, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00,
  0x11, 0x00, 0x00, 0x00
};

if (send_netlink_raw((unsigned char *)set_mcast22, sizeof(set_mcast22)) < 0) {
    wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
} else {
    wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
}
...
char set_beacon05[] = {
  ,
  ...
  0x04, 0xac, 0x0f, 0x00, 0x10, 0x00, 0x2a, 0x00,
  0x7f, 0x0a, 0x04, 0x00, 0x00, 0x02, 0x01, 0x00,
  0x41, 0x00, 0x00, 0x40, 0x10, 0x00, 0x7f, 0x00,
  0x7f, 0x0a, 0x04, 0x00, 0x00, 0x02, 0x01, 0x00,
  0x41, 0x00, 0x00, 0x40, 0x13, 0x00, 0x80, 0x00,
  0x7f, 0x0a, 0x04, 0x00, 0x00, 0x02, 0x01, 0x00,
  0x41, 0x00, 0x00, 0x40, 0xf4, 0x01, 0x20, 0x00,
  0x08, 0x00, 0x26, 0x00, 0x6c, 0x09, 0x00, 0x00,
  0x08, 0x00, 0x27, 0x00, 0x03, 0x00, 0x00, 0x00,
  0x0c, 0x00, 0x17, 0x81, 0x05, 0x00, 0x06, 0x00,
  0x03, 0x00, 0x00, 0x00, 0x0c, 0x00, 0x1b, 0x81,
...
if (send_netlink_raw((unsigned char *)set_beacon05, sizeof(set_beacon05)) < 0) {
    wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
} else {
    wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
}
...
char set_bss05[] = {
  0x54, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00,
  0x83, 0xd3, 0x6a, 0x62, 0x60, 0x0c, 0x00, 0x00,
  0x19, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00,
  0x11, 0x00, 0x00, 0x00, 0x05, 0x00, 0x1c, 0x00,
  0x00, 0x00, 0x00, 0x00, 0x05, 0x00, 0x1d, 0x00,
  0x01, 0x00, 0x00, 0x00, 0x05, 0x00, 0x1e, 0x00,
  0x01, 0x00, 0x00, 0x00, 0x06, 0x00, 0x6d, 0x00,
  0x00, 0x00, 0x00, 0x00, 0x05, 0x00, 0x60, 0x00,
  0x01, 0x00, 0x00, 0x00, 0x07, 0x00, 0x24, 0x00,
  0x0c, 0x18, 0x30, 0x00, 0x05, 0x00, 0x39, 0x01,
...
if (send_netlink_raw((unsigned char *)set_bss05, sizeof(set_bss05)) < 0) {
    wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
} else {
    wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
}
...
char set_mcast25[] = {
  0x1c, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00,
  0x84, 0xd3, 0x6a, 0x62, 0x60, 0x0c, 0x00, 0x00,
  0x79, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00,
  0x11, 0x00, 0x00, 0x00
};

if (send_netlink_raw((unsigned char *)set_mcast25, sizeof(set_mcast25)) < 0) {
    wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
} else {
    wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
}
...
char set_beacon15[] = {
  0x74, 0x02, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00,
  0x2c, 0xd3, 0x6a, 0x62, 0x60, 0x0c, 0x80, 0x00,
  0x0e, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00,
  0x11, 0x00, 0x00, 0x00, 0x3f, 0x00, 0x0e, 0x00,
  0x80, 0x00, 0x00, 0x00, 0xff, 0xff, 0xff, 0xff,
  0xff, 0xff, 0x00, 0x11, 0x22, 0x33, 0x44, 0x01,
  ...
  0x37, 0x36, 0x5f, 0x41, 0x50, 0x5f, 0x4d, 0x4c,
  0x44, 0x01, 0x08, 0x8c, 0x12, 0x98, 0x24, 0xb0,
  0x48, 0x60, 0x6c, 0x00, 0x31, 0x01, 0x0f, 0x00,
  0x30, 0x14, 0x01, 0x00, 0x00, 0x0f, 0xac, 0x04,
  0x01, 0x00, 0x00, 0x0f, 0xac, 0x04, 0x01, 0x00,
  0x00, 0x0f, 0xac, 0x08, 0xcc, 0x00, 0x0b, 0x05,
  0x00, 0x00, 0x00, 0x00, 0x00, 0x3b, 0x02, 0x80,
  ...
  ,
  ...
  0x08, 0x00, 0x4a, 0x00, 0x04, 0xac, 0x0f, 0x00,
  0x10, 0x00, 0x2a, 0x00, 0x7f, 0x0a, 0x04, 0x00,
  0x00, 0x02, 0x01, 0x00, 0x41, 0x40, 0x00, 0x40,
  0x10, 0x00, 0x7f, 0x00, 0x7f, 0x0a, 0x04, 0x00,
  0x00, 0x02, 0x01, 0x00, 0x41, 0x40, 0x00, 0x40,
  0x13, 0x00, 0x80, 0x00, 0x7f, 0x0a, 0x04, 0x00,
  0x00, 0x02, 0x01, 0x00, 0x41, 0x40, 0x00, 0x40,
  ...
  0x3c, 0x14, 0x00, 0x00, 0x08, 0x00, 0x9f, 0x00,
  0x03, 0x00, 0x00, 0x00, 0x08, 0x00, 0xa0, 0x00,
  0x5a, 0x14, 0x00, 0x00, 0x0c, 0x00, 0x17, 0x81,
  0x05, 0x00, 0x06, 0x00, 0x03, 0x00, 0x00, 0x00,
  0x0c, 0x00, 0x1b, 0x81, 0x05, 0x00, 0x01, 0x00,
  0x02, 0x00, 0x00, 0x00, 0x04, 0x00, 0x16, 0x01,
  0x05, 0x00, 0x2a, 0x01, 0x03, 0x00, 0x00, 0x00,
  0x04, 0x00, 0xcc, 0x00
};

if (send_netlink_raw((unsigned char *)set_beacon15, sizeof(set_beacon15)) < 0) {
    wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
} else {
    wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
}
...
char set_bss15[] = {
  0x4c, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00,
  0x85, 0xd3, 0x6a, 0x62, 0x60, 0x0c, 0x00, 0x00,
  0x19, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00,
  0x11, 0x00, 0x00, 0x00, 0x05, 0x00, 0x1c, 0x00,
  0x00, 0x00, 0x00, 0x00, 0x05, 0x00, 0x1d, 0x00,
  0x01, 0x00, 0x00, 0x00, 0x06, 0x00, 0x6d, 0x00,
...
if (send_netlink_raw((unsigned char *)set_bss15, sizeof(set_bss15)) < 0) {
    wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
} else {
    wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
}
...
char set_mcast26[] = {
  0x1c, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00,
  0x86, 0xd3, 0x6a, 0x62, 0x60, 0x0c, 0x00, 0x00,
  0x79, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00,
  0x11, 0x00, 0x00, 0x00
};

if (send_netlink_raw((unsigned char *)set_mcast26, sizeof(set_mcast26)) < 0) {
    wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
} else {
    wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
}
...
char new_key26[] = {
  0x4c, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00,
  0x87, 0xd3, 0x6a, 0x62, 0x60, 0x0c, 0x00, 0x00,
  0x0b, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00,
  0x11, 0x00, 0x00, 0x00, 0x28, 0x00, 0x50, 0x00,
  0x14, 0x00, 0x01, 0x00, 0x21, 0x2b, 0xfa, 0x23,
  0x6b, 0x1b, 0x64, 0x60, 0xf5, 0xf4, 0x4a, 0x36,
  0xd4, 0x7b, 0x44, 0xf1, 0x08, 0x00, 0x03, 0x00,
  0x04, 0xac, 0x0f, 0x00, 0x05, 0x00, 0x02, 0x00,
  0x01, 0x00, 0x00, 0x00, 0x05, 0x00, 0x39, 0x01,
  0x02, 0x00, 0x00, 0x00
};

if (send_netlink_raw((unsigned char *)new_key26, sizeof(new_key26)) < 0) {
    wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
} else {
    wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
}
...
char set_key26[] = {
  0x3c, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00,
  0x88, 0xd3, 0x6a, 0x62, 0x60, 0x0c, 0x00, 0x00,
  0x0a, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00,
  0x11, 0x00, 0x00, 0x00, 0x18, 0x00, 0x50, 0x00,
  0x05, 0x00, 0x02, 0x00, 0x01, 0x00, 0x00, 0x00,
  0x04, 0x00, 0x05, 0x00, 0x08, 0x00, 0x08, 0x80,
...
if (send_netlink_raw((unsigned char *)set_key26, sizeof(set_key26)) < 0) {
    wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
} else {
    wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
}
...
char new_key27[] = {
  0x4c, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00,
  0x89, 0xd3, 0x6a, 0x62, 0x60, 0x0c, 0x00, 0x00,
  0x0b, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00,
  0x11, 0x00, 0x00, 0x00, 0x28, 0x00, 0x50, 0x00,
  0x14, 0x00, 0x01, 0x00, 0x70, 0xa3, 0x21, 0x57,
  0xeb, 0xcf, 0xd3, 0xd9, 0xbe, 0xa1, 0xc0, 0xdd,
  0x73, 0xad, 0x6d, 0x16, 0x08, 0x00, 0x03, 0x00,
  0x06, 0xac, 0x0f, 0x00, 0x05, 0x00, 0x02, 0x00,
  0x04, 0x00, 0x00, 0x00, 0x05, 0x00, 0x39, 0x01,
  0x02, 0x00, 0x00, 0x00
};

if (send_netlink_raw((unsigned char *)new_key27, sizeof(new_key27)) < 0) {
    wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
} else {
    wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
}
...
char set_key27[] = {
  0x3c, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00,
  0x8a, 0xd3, 0x6a, 0x62, 0x60, 0x0c, 0x00, 0x00,
  0x0a, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00,
  0x11, 0x00, 0x00, 0x00, 0x18, 0x00, 0x50, 0x00,
  0x05, 0x00, 0x02, 0x00, 0x04, 0x00, 0x00, 0x00,
  0x04, 0x00, 0x06, 0x00, 0x08, 0x00, 0x08, 0x80,
...
if (send_netlink_raw((unsigned char *)set_key27, sizeof(set_key27)) < 0) {
    wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
} else {
    wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
}
...
char set_wiphy28[] = {
  0x54, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00,
  0x8b, 0xd3, 0x6a, 0x62, 0x60, 0x0c, 0x00, 0x00,
  0x02, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00,
  0x11, 0x00, 0x00, 0x00, 0x30, 0x00, 0x25, 0x80,
  0x2c, 0x00, 0x01, 0x80, 0x05, 0x00, 0x01, 0x00,
  0x00, 0x00, 0x00, 0x00, 0x06, 0x00, 0x02, 0x00,
...
if (send_netlink_raw((unsigned char *)set_wiphy28, sizeof(set_wiphy28)) < 0) {
    wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
} else {
    wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
}
...
char set_wiphy29[] = {
  0x54, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00,
  0x8c, 0xd3, 0x6a, 0x62, 0x60, 0x0c, 0x00, 0x00,
  0x02, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00,
  0x11, 0x00, 0x00, 0x00, 0x30, 0x00, 0x25, 0x80,
  0x2c, 0x00, 0x01, 0x80, 0x05, 0x00, 0x01, 0x00,
  0x01, 0x00, 0x00, 0x00, 0x06, 0x00, 0x02, 0x00,
...
if (send_netlink_raw((unsigned char *)set_wiphy29, sizeof(set_wiphy29)) < 0) {
    wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
} else {
    wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
}
...
char set_wiphy210[] = {
  0x54, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00,
  0x8d, 0xd3, 0x6a, 0x62, 0x60, 0x0c, 0x00, 0x00,
  0x02, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00,
  0x11, 0x00, 0x00, 0x00, 0x30, 0x00, 0x25, 0x80,
  0x2c, 0x00, 0x01, 0x80, 0x05, 0x00, 0x01, 0x00,
  0x02, 0x00, 0x00, 0x00, 0x06, 0x00, 0x02, 0x00,
...
if (send_netlink_raw((unsigned char *)set_wiphy210, sizeof(set_wiphy210)) < 0) {
    wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
} else {
    wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
}
...
char set_wiphy211[] = {
  0x54, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00,
  0x8e, 0xd3, 0x6a, 0x62, 0x60, 0x0c, 0x00, 0x00,
  0x02, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00,
  0x11, 0x00, 0x00, 0x00, 0x30, 0x00, 0x25, 0x80,
  0x2c, 0x00, 0x01, 0x80, 0x05, 0x00, 0x01, 0x00,
  0x03, 0x00, 0x00, 0x00, 0x06, 0x00, 0x02, 0x00,
...
if (send_netlink_raw((unsigned char *)set_wiphy211, sizeof(set_wiphy211)) < 0) {
    wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
} else {
    wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
}
...
char set_vendor_20[] = {
  0x48, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00,
  0x8f, 0xd3, 0x6a, 0x62, 0x60, 0x0c, 0x00, 0x00,
  0x67, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00,
  0x11, 0x00, 0x00, 0x00, 0x08, 0x00, 0xc3, 0x00,
  0xe7, 0x0c, 0x00, 0x00, 0x08, 0x00, 0xc4, 0x00,
  0xcc, 0x00, 0x00, 0x00, 0x1c, 0x00, 0xc5, 0x80,
...
if (send_netlink_raw((unsigned char *)set_vendor_20, sizeof(set_vendor_20)) < 0) {
    wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
} else {
    wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
}
...
char set_vendor_21[] = {
  0x48, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00,
  0x90, 0xd3, 0x6a, 0x62, 0x60, 0x0c, 0x00, 0x00,
  0x67, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00,
  0x11, 0x00, 0x00, 0x00, 0x08, 0x00, 0xc3, 0x00,
  0xe7, 0x0c, 0x00, 0x00, 0x08, 0x00, 0xc4, 0x00,
  0xc7, 0x00, 0x00, 0x00, 0x1c, 0x00, 0xc5, 0x80,
...
if (send_netlink_raw((unsigned char *)set_vendor_21, sizeof(set_vendor_21)) < 0) {
    wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
} else {
    wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
}
...
if (send_netlink_raw((unsigned char *)set_vendor_22, sizeof(set_vendor_22)) < 0) {
    wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
} else {
    wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
}
...
char set_vendor_23[] = {
  0x40, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00,
  0x91, 0xd3, 0x6a, 0x62, 0x60, 0x0c, 0x00, 0x00,
  0x67, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00,
  0x11, 0x00, 0x00, 0x00, 0x08, 0x00, 0xc3, 0x00,
  0xe7, 0x0c, 0x00, 0x00, 0x08, 0x00, 0xc4, 0x00,
  0xc5, 0x00, 0x00, 0x00, 0x14, 0x00, 0xc5, 0x80,
...
if (send_netlink_raw((unsigned char *)set_vendor_23, sizeof(set_vendor_23)) < 0) {
    wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
} else {
    wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
}
...
char set_vendor_24[] = {
  0x38, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00,
  0x92, 0xd3, 0x6a, 0x62, 0x60, 0x0c, 0x00, 0x00,
  0x67, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00,
  0x11, 0x00, 0x00, 0x00, 0x08, 0x00, 0xc3, 0x00,
  0xe7, 0x0c, 0x00, 0x00, 0x08, 0x00, 0xc4, 0x00,
  0xc8, 0x00, 0x00, 0x00, 0x0c, 0x00, 0xc5, 0x80,
...
if (send_netlink_raw((unsigned char *)set_vendor_24, sizeof(set_vendor_24)) < 0) {
    wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
} else {
    wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
}
...
char set_vendor_25[] = {
  0x38, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00,
  0x93, 0xd3, 0x6a, 0x62, 0x60, 0x0c, 0x00, 0x00,
  0x67, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00,
  0x11, 0x00, 0x00, 0x00, 0x08, 0x00, 0xc3, 0x00,
  0xe7, 0x0c, 0x00, 0x00, 0x08, 0x00, 0xc4, 0x00,
  0xc9, 0x00, 0x00, 0x00, 0x0c, 0x00, 0xc5, 0x80,
...
if (send_netlink_raw((unsigned char *)set_vendor_25, sizeof(set_vendor_25)) < 0) {
    wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
} else {
    wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
}
...
char set_vendor_26[] = {
  0x38, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00,
  0x94, 0xd3, 0x6a, 0x62, 0x60, 0x0c, 0x00, 0x00,
  0x67, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00,
  0x11, 0x00, 0x00, 0x00, 0x08, 0x00, 0xc3, 0x00,
  0xe7, 0x0c, 0x00, 0x00, 0x08, 0x00, 0xc4, 0x00,
  0xc4, 0x00, 0x00, 0x00, 0x0c, 0x00, 0xc5, 0x80,
...
if (send_netlink_raw((unsigned char *)set_vendor_26, sizeof(set_vendor_26)) < 0) {
    wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
} else {
    wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
}
...
char set_vendor_27[] = {
  0x58, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00,
  0x95, 0xd3, 0x6a, 0x62, 0x60, 0x0c, 0x00, 0x00,
  0x67, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00,
  0x11, 0x00, 0x00, 0x00, 0x08, 0x00, 0xc3, 0x00,
  0xe7, 0x0c, 0x00, 0x00, 0x08, 0x00, 0xc4, 0x00,
  0xce, 0x00, 0x00, 0x00, 0x2c, 0x00, 0xc5, 0x80,
...
if (send_netlink_raw((unsigned char *)set_vendor_27, sizeof(set_vendor_27)) < 0) {
    wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
} else {
    wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
}
...
char set_beacon500[] = {
  0xa0, 0x02, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00,
  0x2d, 0xd3, 0x6a, 0x62, 0x60, 0x0c, 0x80, 0x00,
  0x0e, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00,
  0x11, 0x00, 0x00, 0x00, 0x3f, 0x00, 0x0e, 0x00,
  0x80, 0x00, 0x00, 0x00, 0xff, 0xff, 0xff, 0xff,
  0xff, 0xff, 0x00, 0x11, 0x22, 0x33, 0x44, 0x02,
...
if (send_netlink_raw((unsigned char *)set_beacon500, sizeof(set_beacon500)) < 0) {
    wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
} else {
    wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
}
...
char set_bss501[] = {
  0x44, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00,
  0x96, 0xd3, 0x6a, 0x62, 0x60, 0x0c, 0x00, 0x00,
  0x19, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00,
  0x11, 0x00, 0x00, 0x00, 0x05, 0x00, 0x1c, 0x00,
  0x00, 0x00, 0x00, 0x00, 0x05, 0x00, 0x1d, 0x00,
  0x01, 0x00, 0x00, 0x00, 0x05, 0x00, 0x60, 0x00,
...
if (send_netlink_raw((unsigned char *)set_bss501, sizeof(set_bss501)) < 0) {
    wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
} else {
    wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
}
...
char set_mcast502[] = {
  0x1c, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00,
  0x97, 0xd3, 0x6a, 0x62, 0x60, 0x0c, 0x00, 0x00,
  0x79, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00,
  0x11, 0x00, 0x00, 0x00
};

if (send_netlink_raw((unsigned char *)set_mcast502, sizeof(set_mcast502)) < 0) {
    wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
} else {
    wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
}
...
char set_beacon503[] = {
  0x58, 0x02, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00,
  0x2e, 0xd3, 0x6a, 0x62, 0x60, 0x0c, 0x80, 0x00,
  0x0e, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00,
  0x11, 0x00, 0x00, 0x00, 0x42, 0x00, 0x0e, 0x00,
  0x80, 0x00, 0x00, 0x00, 0xff, 0xff, 0xff, 0xff,
  0xff, 0xff, 0x00, 0x11, 0x22, 0x33, 0x44, 0x00,
...
if (send_netlink_raw((unsigned char *)set_beacon503, sizeof(set_beacon503)) < 0) {
    wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
} else {
    wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
}
...
char set_bss504[] = {
  0x54, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00,
  0x98, 0xd3, 0x6a, 0x62, 0x60, 0x0c, 0x00, 0x00,
  0x19, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00,
  0x11, 0x00, 0x00, 0x00, 0x05, 0x00, 0x1c, 0x00,
  0x00, 0x00, 0x00, 0x00, 0x05, 0x00, 0x1d, 0x00,
  0x01, 0x00, 0x00, 0x00, 0x05, 0x00, 0x1e, 0x00,
...
if (send_netlink_raw((unsigned char *)set_bss504, sizeof(set_bss504)) < 0) {
    wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
} else {
    wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
}
...
char set_mcast505[] = {
  0x1c, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00,
  0x99, 0xd3, 0x6a, 0x62, 0x60, 0x0c, 0x00, 0x00,
  0x79, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00,
  0x11, 0x00, 0x00, 0x00
};

if (send_netlink_raw((unsigned char *)set_mcast505, sizeof(set_mcast505)) < 0) {
    wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
} else {
    wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
}
...
char set_beacon506[] = {
  0x74, 0x02, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00,
  0x2f, 0xd3, 0x6a, 0x62, 0x60, 0x0c, 0x80, 0x00,
  0x0e, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00,
  0x11, 0x00, 0x00, 0x00, 0x3f, 0x00, 0x0e, 0x00,
  0x80, 0x00, 0x00, 0x00, 0xff, 0xff, 0xff, 0xff,
  0xff, 0xff, 0x00, 0x11, 0x22, 0x33, 0x44, 0x01,
...
if (send_netlink_raw((unsigned char *)set_beacon506, sizeof(set_beacon506)) < 0) {
    wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
} else {
    wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
}
...
char set_bss_507[] = {
  0x4c, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00,
  0x9a, 0xd3, 0x6a, 0x62, 0x60, 0x0c, 0x00, 0x00,
  0x19, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00,
  0x11, 0x00, 0x00, 0x00, 0x05, 0x00, 0x1c, 0x00,
  0x00, 0x00, 0x00, 0x00, 0x05, 0x00, 0x1d, 0x00,
  0x01, 0x00, 0x00, 0x00, 0x06, 0x00, 0x6d, 0x00,
...
if (send_netlink_raw((unsigned char *)set_bss_507, sizeof(set_bss_507)) < 0) {
    wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
} else {
    wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
}
...
char set_mcast_508[] = {
  0x1c, 0x00, 0x00, 0x00, 0x19, 0x00, 0x05, 0x00,
  0x9b, 0xd3, 0x6a, 0x62, 0x60, 0x0c, 0x00, 0x00,
  0x79, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00,
  0x11, 0x00, 0x00, 0x00
};

if (send_netlink_raw((unsigned char *)set_mcast_508, sizeof(set_mcast_508)) < 0) {
    wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
} else {
    wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
}
...
char probe_resp24[] = {
  ,
...
char probe_resp5[] = {
  
};

char assoc_resp5[] = {
  ,
  ...
  ,
  ...
  ,
                                  ...
                                  size_t csa_offs_len)
{
    ...
    // if (interface->vap_info.vap_index == 0 && WLAN_FC_GET_TYPE(fc) == WLAN_FC_TYPE_MGMT &&
    //     WLAN_FC_GET_STYPE(fc) == WLAN_FC_STYPE_PROBE_RESP) {
    //     if (send_netlink_raw((unsigned char *)probe_resp24, sizeof(probe_resp24)) < 0) {
    //         wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
    //     } else {
    //         wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
    //     }
    ...
    // if (interface->vap_info.vap_index == 1 && WLAN_FC_GET_TYPE(fc) == WLAN_FC_TYPE_MGMT &&
    //     WLAN_FC_GET_STYPE(fc) == WLAN_FC_STYPE_PROBE_RESP) {
    //     if (send_netlink_raw((unsigned char *)probe_resp5, sizeof(probe_resp5)) < 0) {
    //         wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
    //     } else {
    //         wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
    //     }
    ...
    //     if (send_netlink_raw((unsigned char *)assoc_resp5, sizeof(assoc_resp5)) < 0) {
    //         wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
    //     } else {
    //         wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
    //     }
    ...
    ret = nl_send_auto_complete((struct nl_sock *)interface->bss_nl_connect_event, msg);
    msg = NULL;
    if (ret) {
...
#endif
{
    ...
    //XXX
    msg = nl80211_drv_cmd_msg(g_wifi_hal.nl80211_id, interface, 0, NL80211_CMD_SET_WIPHY);
    ...
    nl80211_send_and_recv(msg, NULL, NULL, NULL, NULL);
...
//XXX
int wifi_drv_sta_set_flags_(void *priv, const u8 *addr,
                        ...
                        unsigned int flags_and)
{
    ...
    //XXX
    if (flags_or == 0) {
        wifi_drv_sta_set_flags_(priv, addr, total_flags, flags_or, flags_and);
    }
...
char eapol[] = {
  
};
    ...
    u16 proto, const u8 *buf, size_t len, int no_encrypt)
{
    ...
    // //XXX
    // send_netlink_raw(eapol, sizeof(eapol));
    // return 0;
    ...
    //XXX return nl80211_send_and_recv(msg, NULL, &g_wifi_hal, NULL, NULL);
    return execute_send_and_recv(interface->bss_nl_cb, interface->bss_nl_connect_event, msg,
                    NULL, NULL, NULL, NULL);
}
...
char add_link_sta[] = {
  
};

char link_sta_mac[] = { 0xee, 0x78, 0x05, 0xc1, 0x88, 0x09 };
char set_sta[] = {
  
};


void replace_bytes(char *buffer, size_t buffer_len,
    ...
    for (size_t i = 0; i <= buffer_len - pattern_len; ++i) {
        if (memcmp(&buffer[i], pattern, pattern_len) == 0) {
            ...
            // If replacement is shorter, zero out the remaining bytes
            if (replacement_len < pattern_len) {
                memset(&buffer[i + replacement_len], 0, pattern_len - replacement_len);
            }
            // Skip ahead if you only want to replace the first match
            // i += pattern_len - 1;
        }
    }
}
...
#endif /* CONFIG_IEEE80211BE */
wifi_hal_dbg_print("%s:%d: === link id: %d\n", __func__, __LINE__, params->mld_link_id);
        //TODO: config
...
fail:
    ...
    // wifi_hal_dbg_print("%s:%d: ===\n", __func__, __LINE__);
    // if (params->set && params->mld_link_id >= 0) {
    //     params->mld_link_id = -1;
    //     wifi_hal_dbg_print("%s:%d: ===\n", __func__, __LINE__);
    //     wifi_drv_sta_add(priv, params);
    // }

    // if (cmd == NL80211_CMD_NEW_STATION && params->mld_link_addr && params->mld_link_id == 1) {
    //     wifi_hal_dbg_print("%s:%d: ===add link_sta_mac for link 0\n", __func__, __LINE__);
    //     send_netlink_raw(add_link_sta, sizeof(add_link_sta));

    //     replace_bytes(set_sta, sizeof(set_sta), link_sta_mac, sizeof(link_sta_mac),
    //         params->mld_link_addr, 6);
    //     send_netlink_raw(set_sta, sizeof(set_sta));
    // }
...
static int nl80211_put_control_port(wifi_driver_data_t *drv,
				    struct nl_msg *msg)
{
...
int wifi_drv_set_ap(void *priv, struct wpa_driver_ap_params *params)
{
    ...
    nl80211_put_control_port(drv, msg);
...
#ifdef EAPOL_OVER_NL
    //XXX
    wifi_interface_info_t *mld = wifi_hal_get_mld_interface_by_link_id(interface, 0);
    if (g_wifi_hal.platform_flags & PLATFORM_FLAGS_CONTROL_PORT_FRAME && mld->bss_nl_connect_event_fd > 0 ) {
        ret = nl80211_set_rx_control_port_owner(msg, mld);
        if (ret) {
            wifi_hal_error_print("%s:%d: failed to set rx control port owner on interface %s, "
                    "error: %d (%s)\n", __func__, __LINE__, mld->name, ret, strerror(-ret));
            return -1;
        }


    if (interface->vap_info.vap_index == 16) {
        mld->bss_allow_recv = 1;
    }
...
char packet_bytes[] = {
  0xc9, 0x3c, 0x00, 0x10, 0x86, 0x25, 0xff, 0x00,
  0x0c, 0x43, 0x26, 0x60, 0x31, 0xff, 0x62, 0x81,
  0x23, 0x40, 0x7f, 0xff, 0xff, 0x0f, 0x00, 0x10,
  0x81, 0x24, 0xff, 0x00, 0x0c, 0x43, 0x26, 0x60,
  0x20, 0x6c, 0x3f, 0x8d, 0x20, 0x42, 0x7f, 0x00,
  0x01, 0x00, 0x00, 0x10, 0x86, 0x25, 0xff, 0x00,
  0x0c, 0x43, 0x26, 0x60, 0x30, 0x6c, 0x3f, 0x8d,
  0x20, 0x42, 0x7f, 0x00, 0x02, 0x00
};
...
static u8* wifi_drv_get_rnr_colocation_ie(void *priv, u8 *eid, size_t *current_len)
{
    ...
    //XXX
    memcpy(eid, packet_bytes, sizeof(packet_bytes));
    eid += sizeof(packet_bytes);
    return eid;
