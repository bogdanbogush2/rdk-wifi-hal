             ...
             void *valid_finish_data)
{
    ...
    wifi_interface_info_t *interface = valid_finish_data;
    uint8_t ifid = if_nametoindex("mld0");

    if (cmd == NL80211_CMD_STOP_AP) {
        // ignore
        err = 0;
        goto out;
    }
...
char set_wiphy0[] = {
  ...
  ifid, 0x00, 0x00, 0x00, 0x08, 0x00, 0x26, 0x00,
...
// before del station
if (cmd == NL80211_CMD_DEL_STATION && interface && interface->vap_info.vap_index == 0) {
...
char del_station0[] = {
  ...
  ifid, 0x00, 0x00, 0x00, 0x05, 0x00, 0x39, 0x01,
  0x00, 0x00, 0x00, 0x00
};
...
char qos_map0[] = {
  ...
  ifid, 0x00, 0x00, 0x00, 0x16, 0x00, 0xc7, 0x00,
  0x00, 0x00, 0x02, 0x10, 0x01, 0x01, 0xff, 0xff,
  0x12, 0x16, 0x18, 0x26, 0x28, 0x28, 0x2c, 0x2e,
  0x30, 0x38, 0x00, 0x00
};
// before channel
if (cmd == NL80211_CMD_SET_CHANNEL && interface && interface->vap_info.vap_index == 0) {
...
char start_ap0[] = {
  ...
  ifid, 0x00, 0x00, 0x00, 0x46, 0x00, 0x0e, 0x00,
  ...
  0x03, 0x00, 0x00, 0x00, /*0x04, 0x00, 0xcc, 0x00*/
    ...
    if (cmd == NL80211_CMD_NEW_BEACON && interface && interface->vap_info.vap_index == 0) {
...
// before channel
if (cmd == NL80211_CMD_SET_CHANNEL && interface && interface->vap_info.vap_index == 1) {
...
// before channel
if (cmd == NL80211_CMD_SET_CHANNEL && interface && interface->vap_info.vap_index == 1) {
    if (send_netlink_raw((unsigned char *)qos_map1, sizeof(qos_map1)) < 0) {
        wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
    } else {
        wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
    }
...
char start_ap1[] = {
  ...
  0x0f, 0x00, 0x00, 0x00, 0x08, 0x00, 0x03, 0x00,
  ifid, 0x00, 0x00, 0x00, 0x43, 0x00, 0x0e, 0x00,
  0x80, 0x00, 0x00, 0x00, 0xff, 0xff, 0xff, 0xff,
  ...
  0x03, 0x00, 0x00, 0x00, /*0x04, 0x00, 0xcc, 0x00*/
    ...
    if (cmd == NL80211_CMD_NEW_BEACON && interface && interface->vap_info.vap_index == 1) {
...
// before channel
if (cmd == NL80211_CMD_SET_CHANNEL && interface && interface->vap_info.vap_index == 16) {
...
// before channel
if (cmd == NL80211_CMD_SET_CHANNEL && interface && interface->vap_info.vap_index == 16) {
    if (send_netlink_raw((unsigned char *)qos_map2, sizeof(qos_map2)) < 0) {
        wifi_hal_error_print("%s:%d: Failed to set\n", __func__, __LINE__);
    } else {
        wifi_hal_info_print("%s:%d: Successfully set\n", __func__, __LINE__);
    }
...
char start_ap2[] = {
  ...
  0x58, 0x0a, 0x00, 0x00, /*0x04, 0x00, 0xcc, 0x00*/
    ...
    if (cmd == NL80211_CMD_NEW_BEACON && interface && interface->vap_info.vap_index == 16) {
...
int nl80211_kick_device(wifi_interface_info_t *interface, mac_address_t addr)
{
    ...
    if (nl80211_send_and_recv(msg, kick_device_handler, interface, NULL, interface)) {
...
//    if (!radio->configured && link_id != NL80211_DRV_LINK_ID_NA &&
...
#endif
    if ((ret = nl80211_send_and_recv(msg, wiphy_set_info_handler, &g_wifi_hal, NULL, interface))) {
    ...
    const u8 *addr, int deauth, u16 reason_code)
{
    ...
    ret = nl80211_send_and_recv(msg, NULL, NULL, NULL, interface);
    if (ret < 0) {
...
#endif
    ret = nl80211_send_and_recv(msg, beacon_info_handler, &g_wifi_hal, NULL, interface);
    if (ret != 0) {
